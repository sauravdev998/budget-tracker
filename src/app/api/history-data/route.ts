...
async function getHistoryData(
  ...
  switch (timeframe) {
    ...
    case "month":
      return await getMonthHistoryDate(userId, period.year, period.month);
  }
}
type HistoryData = {
  expense: number;
...
async function getYearHistoryData(userId: string, year: number) {
  ...
  for (let i = 0; i < 12; i++) {
    let income = 0;
    let expense = 0;
    const month = result.find((e) => e.month === i);
    if (month) {
...
async function getMonthHistoryDate(
  ...
  if (!result || result.length === 0) return [];
  const history: HistoryData[] = [];
  const daysInMunth = getDaysInMonth(new Date(year, month));

  for (let i = 0; i < daysInMunth; i++) {
    let income = 0;
    let expense = 0;
